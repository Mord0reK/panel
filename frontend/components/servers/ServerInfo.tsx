...
export function ServerInfo({ server }: ServerInfoProps) {
  ...
  return (
    <div className="flex flex-col items-start gap-3 rounded-lg border border-zinc-800 bg-zinc-900/50 px-3 py-3 sm:flex-row sm:items-center sm:gap-4 sm:px-5 sm:py-4">
      {/* Ikona serwera */}
      ...
      <div className="min-w-0 flex-1">
        <h2 className="truncate text-sm font-semibold text-zinc-100 sm:text-base">{displayName}</h2>

        <div className="mt-1 flex flex-wrap items-center gap-y-2 text-xs text-zinc-400 sm:text-sm">
          {/* Status online */}
          {server.online !== undefined && (
            <>
              ...
              {stats.length > 0 && (
                  <span className="mx-2.5 hidden text-zinc-700 sm:inline">|</span>
              )}
            </>
          )}
          ...
          {stats.map((item, i) => (
              <span key={i} className="flex min-w-0 items-center">
                ...
                <item.icon className="size-3.5 shrink-0 text-zinc-500" />
                  <span className="truncate text-zinc-400">{item.value}</span>
              </span>
              {i < stats.length - 1 && (
                  <span className="mx-2.5 hidden text-zinc-700 sm:inline">|</span>
              )}
